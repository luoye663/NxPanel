...
func (s *Server) setupRoutes() {
	...
	s.router.Route("/api/v1/{gateSecret}", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Put("/sites/{site_id}/geo-access", s.handleSiteGeoAccessUpdate)
			r.Post("/sites/{site_id}/geo-access/enable", s.handleSiteGeoAccessEnable)
			r.Post("/sites/{site_id}/geo-access/disable", s.handleSiteGeoAccessDisable)
			r.Get("/sites/{site_id}/geo-rules", s.handleSiteGeoRuleList)
			r.Post("/sites/{site_id}/geo-rules", s.handleSiteGeoRuleCreate)
			r.Put("/sites/{site_id}/geo-rules/order", s.handleSiteGeoRuleReorder)
			r.Put("/sites/{site_id}/geo-rules/{rule_id}", s.handleSiteGeoRuleUpdate)
			r.Delete("/sites/{site_id}/geo-rules/{rule_id}", s.handleSiteGeoRuleDelete)
