...
func (s *Server) setupRoutes() {
	...
	s.router.Route("/api/v1/{gateSecret}", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// Access Limit
			r.Get("/sites/{site_id}/access-policy", s.handleAccessPolicyGet)
			r.Put("/sites/{site_id}/access-policy", s.handleAccessPolicySave)
			r.Post("/sites/{site_id}/access-policy/preview", s.handleAccessPolicyPreview)
			r.Post("/sites/{site_id}/access-policy/sync", s.handleAccessPolicySync)
			...
			r.Post("/sites/{site_id}/auth-rules", s.guardLegacyAccess(s.handleAuthRuleCreate))
			r.Put("/sites/{site_id}/auth-rules/{rule_id}", s.guardLegacyAccess(s.handleAuthRuleUpdate))
			r.Delete("/sites/{site_id}/auth-rules/{rule_id}", s.guardLegacyAccess(s.handleAuthRuleDelete))
			r.Get("/sites/{site_id}/deny-rules", s.handleDenyRuleList)
			r.Post("/sites/{site_id}/deny-rules", s.guardLegacyAccess(s.handleDenyRuleCreate))
			r.Put("/sites/{site_id}/deny-rules/{rule_id}", s.guardLegacyAccess(s.handleDenyRuleUpdate))
			r.Delete("/sites/{site_id}/deny-rules/{rule_id}", s.guardLegacyAccess(s.handleDenyRuleDelete))
			r.Get("/sites/{site_id}/ip-limit-rules", s.handleIPLimitRuleList)
			r.Post("/sites/{site_id}/ip-limit-rules", s.guardLegacyAccess(s.handleIPLimitRuleCreate))
			r.Put("/sites/{site_id}/ip-limit-rules/{rule_id}", s.guardLegacyAccess(s.handleIPLimitRuleUpdate))
			r.Delete("/sites/{site_id}/ip-limit-rules/{rule_id}", s.guardLegacyAccess(s.handleIPLimitRuleDelete))
			r.Get("/geoip/settings", s.handleGeoIPSettingsGet)
			r.Put("/geoip/settings", s.handleGeoIPSettingsUpdate)
			r.Get("/geoip/status", s.handleGeoIPStatus)
			r.Post("/geoip/update", s.handleGeoIPUpdate)
			r.Post("/geoip/database", s.handleGeoIPDatabaseUpload)
			r.Get("/sites/{site_id}/geo-access", s.handleSiteGeoAccessGet)
			r.Put("/sites/{site_id}/geo-access", s.guardLegacyAccess(s.handleSiteGeoAccessUpdate))
			r.Post("/sites/{site_id}/geo-access/enable", s.guardLegacyAccess(s.handleSiteGeoAccessEnable))
			r.Post("/sites/{site_id}/geo-access/disable", s.guardLegacyAccess(s.handleSiteGeoAccessDisable))
			r.Get("/sites/{site_id}/geo-rules", s.handleSiteGeoRuleList)
			r.Post("/sites/{site_id}/geo-rules", s.guardLegacyAccess(s.handleSiteGeoRuleCreate))
			r.Put("/sites/{site_id}/geo-rules/order", s.guardLegacyAccess(s.handleSiteGeoRuleReorder))
			r.Put("/sites/{site_id}/geo-rules/{rule_id}", s.guardLegacyAccess(s.handleSiteGeoRuleUpdate))
			r.Delete("/sites/{site_id}/geo-rules/{rule_id}", s.guardLegacyAccess(s.handleSiteGeoRuleDelete))
			r.Get("/sites/{site_id}/hotlink-rules", s.handleHotlinkRuleList)
			r.Post("/sites/{site_id}/hotlink-rules", s.guardLegacyAccess(s.handleHotlinkRuleCreate))
			r.Put("/sites/{site_id}/hotlink-rules/{rule_id}", s.guardLegacyAccess(s.handleHotlinkRuleUpdate))
			r.Delete("/sites/{site_id}/hotlink-rules/{rule_id}", s.guardLegacyAccess(s.handleHotlinkRuleDelete))
